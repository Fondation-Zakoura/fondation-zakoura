...
class ProjectController extends Controller
{
    ...
    public function store(StoreProjectRequest $request): JsonResponse
    {
        ...
        try {
            ...
            $bankAccountId = $validatedData['project_bank_account_id'] ?? [];
            $createdById = $validatedData['created_by_id']; // later
            unset($validatedData['responsible_id'], $validatedData['project_bank_account_id'], $validatedData['created_by_id']);

            $project = $this->projectService->createProject($validatedData, $responsableId, $bankAccountId,$createdById);
            return response()->json([
                'message' => 'Projet créé avec succès.',
                'project' => $project->load(['createdBy','projectBankAccount' ,'responsible']), // later
            ], 201);
        } catch (Exception $e) {
            return response()->json([
    ...
    public function update(UpdateProjectRequest $request, Project $project): JsonResponse
    {
        ...
        try {
            $responsableId = $validatedData['responsible_id'];
            $bankAccountId = $validatedData['project_bank_account_id'] ?? [];
            unset($validatedData['responsible_id'], $validatedData['project_bank_account_id']);
            $updatedProject = $this->projectService->updateProject($project, $validatedData, $responsableId, $bankAccountId);

            return response()->json([
                'message' => 'Projet mis à jour avec succès.',
                'project' => $updatedProject->load(['responsible', 'createdBy', 'projectBankAccount']),
            ]);
        } catch (Exception $e) {
            return response()->json([
